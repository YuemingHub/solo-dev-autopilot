...
> 新手最常卡在这一步：面对 A/B/C 三个组合不知道选哪个。
> 这个向导通过 3 个简单问题，自动推荐最合适的组合。
> **核心原则：不知道选什么就用 A，A 永远是最安全的选择。**

#### 向导流程

问用户以下 3 个问题（每次只问一个，不要一次性全问）：

**问题 1：你的后端用什么语言？**
- A) TypeScript / JavaScript → 走组合 A 或 B
- B) Python → 走组合 C
- C) 不知道 / 无所谓 → 默认走组合 A

**问题 2（仅 TS/JS）：你追求生态还是极速？**
- A) 生态大、学习资源多 → 组合 A（React + Hono）
- B) 追求极致性能和轻量 → 组合 B（SvelteKit + Elysia）
- C) 不知道 / 无所谓 → 组合 A

**问题 3：你的数据库需要自己管理吗？**
- A) 不想管，用云服务 → Supabase（组合 A）或 Neon（组合 C）
- B) 想自己跑 → 本地 PostgreSQL + Docker
- C) 不知道 / 无所谓 → Supabase（免费额度够新手用）
...
确认这个选型吗？确认后我会开始生成项目骨架。
（如果不确定，直接说"确认"就好，这是经过验证的最优解。）
```

#### 快速模式

如果用户一开始就说了"用默认的"或"不知道选什么"，直接跳过向导，使用组合 A。
不要让新手在选择上花超过 30 秒。
...
6. **选型不超过30秒**：新手最容易在选型上瘫痪。用交互式向导快速确定，不知道选什么就用组合 A。
