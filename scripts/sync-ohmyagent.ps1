# 一键同步 skill 合集 + 2 个 agent + 全局规则到 ohmyagent 全局配置目录
# 用法:在仓库任意位置执行  powershell -ExecutionPolicy Bypass -File scripts\sync-ohmyagent.ps1
...
$agents = Join-Path $repo 'configs\agents'
...
# 3. 全局规则(统一运行契约)
Copy-Item (Join-Path $repo 'AGENTS.md') (Join-Path $cfg 'AGENTS.md') -Force
