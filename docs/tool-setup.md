...
> 判断标准：能否**原生自动加载** 21 个 SKILL.md。Claude Code 官方格式全兼容，
...
# 进入项目后确认 Skill 被识别
claude
# 输入 /skill 应看到 21 个 Skill（api-designer ~ troubleshoot + creating-forward + env-detect ~ book-experiments）
```
...
见 [codex-setup.md](codex-setup.md)：用 AGENTS.md 指令承载三级权限模型，`.codex/skills/` 加载 21 个 Skill；无需插件市场，不强制安装 superpowers（由 21 个 Skill + 9 步闭环承担方法论）。
...
- Cursor 的 Rules 是**全局生效**的，不适合塞 21 个完整 Skill
...
| 问题 | 解决 |
|------|------|
| 其他工具能自动加载 21 个 Skill 吗？ | 不能。只有 Claude Code 原生支持官方 SKILL.md，其他工具需手动转换 |
